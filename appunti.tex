...
\begin{flushleft}
Si vuole estendere ora il concetto di errore relativo definito nel capitolo precedente applicandolo al caso in cui si vuole lavorare con dati in input che non sono più singoli valori ma bensì dei vettori. 
Si immagini dunque di avere  un vettore \VarMtrx{x} ed una funzione $f(\VarMtrx{x})$,  per rappresentare il nostro vettore all'interno della nostra macchina occorre effettuare delle approssimazioni ed ottenere quello che  è un dato perturbato.
...
Ovviamente la cosa da notare  è che le norme utilizzate per il denominatore e il numeratore devono coincidere come tipo (verranno adoperate solamente norme che abbiamo visto in precedenza: 1,2 e $\infty$).
\end{flushleft}
...
\begin{flushleft}
\begin{defn}
Una norma matriciale \textbf{generalizzata} è una funzione che,  analogamente a quella definita per le norme vettoriali, associa ad una matrice $A$ un numero reale maggiore o uguale a 0 : 
\[ 
	\lVert A \rVert :  \mathbb{R}^{m \times n } \longrightarrow \mathbb{R}_{+} \cup \{0\} 
\]
\end{defn}
Una norma matriciale generalizzata  gode delle seguenti proprietà (anche queste analoghe alla norma vettoriale): 
\begin{itemize}
	\item $\lVert A \rVert \geq 0$ $\forall A \in \mathbb{R}^{m \times n}$.
	\item $\lVert A \rVert = 0$ solamente se $A = 0$, ovvero la matrice è vuota
	\item $\lVert\alpha A\rVert = |\alpha|\lVert A \rVert$ con $\alpha \in \mathbb{R}$
	\item $\lVert A + B \rVert \leq \lVert A \rVert + \lVert B \rVert$ per la disuguaglianza triangolare.
\end{itemize}
...
\]
Avremo che $\lVert AB \rVert = 2$ e invece $\lVert A \rVert  = \lVert B \rVert = 1$
\end{exmp}
...
\[  
	\lVert A\VarMtrx{x} \rVert_{V} \leq \lVert A \rVert_{M}\lVert\VarMtrx{x}\rVert_{V} 
\]
Dove $A$ e $\VarMtrx{x}$ sono rispettivamente una matrice ed un vettore che possono essere moltiplicati tra loro. 
\end{defn}
...
\[ 
	\lVert A \VarMtrx{x}\rVert_{V} \leq C\lVert\VarMtrx{x}\rVert_{V} 
\]
\end{defn}
\end{flushleft}
...
\[ 
	\lVert A \rVert_{\infty} = \max_{\{i=1,...m\}} \sum_{j=1}^{n} |a_{i,j}|
\]
\end{defn}

prendendo come riferimento $A$  siano : 
\begin{itemize}
...
\[ \lVert A \rVert_{\infty} = \max \{ C_{1}, C_{2}, \dots,  C_{m}\}\]

\newpage
\begin{defn}
Si definisce norma matriciale uno la funzione :
\[ \lVert A \rVert_{1} = \max_{\{j=1,...n\}} \sum_{i=1}^{m} |a_{i,j}| \]
\end{defn}
...
\[ 
	\lVert A \rVert_{1} = \max \{ D_{1}, D_{2}, \dots,  D_{m}\}
\]
\end{flushleft}
...
\[ 
	\lVert A \rVert_{2} = \sqrt{\rho(A^{T}A)} 
\]
...
La matrice $M=A^{T}A$ di cui andiamo a calcolare il nostro raggio spettrale gode di due proprietà : 
\begin{enumerate}
	\item E' simmetrica : $M = M^{T}$.  E' facile dimostrare tale proprietà infatti : $M^{T} = (A^{T}A)^{T} = A^{T}A = M$. 
	\item E' semidefinita positiva : $\forall \VarMtrx{x} \in \mathbb{R}^{n} \setminus \{0\}$ si ha che $\VarMtrx{x}^{T}M\VarMtrx{x} \geq 0$.
\end{enumerate}
\end{flushleft}
...
Prendiamo in considerazione la matrice definita precedentemente $A^{T}A$, come sappiamo questa è semidefinita positiva e simmetrica, inoltre possiede n autovalori reali e non nulli.
Possiamo ordinare per valore crescente quest'ultimi e in particolare interessarci dei due aventi valore massimo e minimo: 
\[ \lambda_{max}(A^{T}A) \quad \lambda_{min}(A^{T}A) \]

\begin{defn}
Si definisce numero di condizionamento in norma 2 di una matrice $A$ il valore : 
\[ 
	K_{2}(A)= \frac{\sqrt{ \lambda_{max}(A^{T}A)}}{\sqrt{ \lambda_{min}(A^{T}A)}}
\]
ovvero, il rapporto delle radici dell'autovalore massimo e minimo della matrice  $A^{T}A$
\end{defn}

Possiamo denotare tre proprietà per il numero di condizionamento :

\begin{enumerate}
	\item $K_{2}(A^{T}A) = K_{2}(A)^{2} = \frac{\lambda_{max}(A^{T}A)}{\lambda_{min}(A^{T}A)}$
	\item Se $A \in \mathbb{R}^{n \times n}$ è una matrice non singolare (determinante diverso da zero) allora possiamo riscrivere il suo valore di condizionamento nel seguente modo : 
	\[K_{2}(A) = \lVert A \rVert_{2}\lVert A^{-1} \rVert_{2} \]
	\item Se $A \in \mathbb{R}^{n \times n}$ è una matrice simmetrica allora possiamo riscrivere il suo valore di condizionamento nel seguente modo : 
	\[K_{2}(A) = \frac{\max_{\{i=1,\dots, n\}}|\lambda_{i}(A)|}{\min_{\{i=1,\dots, n\}}|\lambda_{i}(A)|} \]
\end{enumerate}
